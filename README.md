...
- 订单明细增强
  - 「平方(㎡)」列（两位小数，自动由长宽/数量计算）
  - 底部汇总行显示总数量与总方数
  - 打印底部显示 总数量 / 总方数 / 合计金额
- 丝管理（工具栏入口）
  - 独立主数据，与样块及订单完全解耦
  - 字段：型号 / 价格 / 进货时间 / 备注
  - 支持多附件（图片等），文件存 `attachments/wires/<id>/`
  - 筛选：型号 / 价格区间 / 进货时间范围 / 备注
  - 新增、编辑、删除；型号重复会被阻止；删除记录连带删除附件目录
- 样块管理（工具栏入口）
  - 独立主数据，与丝及订单完全解耦
  - 字段：型号 / 客户 / 订单时间 / 备注
  - 支持多附件（图片等），文件存 `attachments/sampleblocks/<id>/`
  - 筛选：型号 / 客户 / 订单时间范围 / 备注
  - 新增、编辑、删除；型号重复会被阻止；删除记录连带删除附件目录
...
- 输入单位
  ...
  - 玻璃单价：元/㎡
- 公式
  - 面积(㎡) = (LengthMm / 1000) \* (WidthMm / 1000)
  - 玻璃费用 = AreaM2 \* Quantity \* GlassUnitPricePerM2
  - 行金额 = 玻璃费用 + HoleFee + OtherFee（注：WireUnitPrice 存储于 OrderItem 但**不计入行金额**）
  - 订单总额 = Sum(各行取整后金额)，再取整
- 精度
  - 全程 decimal
  - `RoundAmount` 取整到 0 位（整元，AwayFromZero）；`Round` 取整到 2 位
